...
int main()
{
	...
	while(bLoop)
	{
		...
		if(!strcmp(pTemp,"exit")) {
			...
			//MapObject->PuTile(x,y,tile_index);
		}
			...
			/*for(int ix=0;ix<MapObject.m_header.m_nWidth;ix++ ) {
			  MapObject.m_pBuf[x * MapObject.m_header.m_nHeight + ix ] = tile_index;
			  }
			  for(int iy=0;iy<MapObject.m_header.m_nHeight+y;iy++ ) {
			  MapObject.m_pBuf[iy* MapObject.m_header.m_nWidth + y ] = tile_index;
			  }*/
		}
	}	
	return 0;
