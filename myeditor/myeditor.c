...
{
	for(int i=0;i< sizeof(proc_objercts)/sizeof(_S_PROC_OBJECT);i++) {
		if( strcmp(proc_objercts[i].m_szToken,szToken) == 0 )
		{
			proc_objercts[i].m_fp();
			break;
		}
	}
}
